...
* **Pushup Form Gating:** Sustained bad back form (`BACK_SAG`) rejects the rep when posture landmarks are available. Back-angle evaluation prefers 3D world landmarks, falls back from ankle to knee when ankles are low-confidence, and enters an inferred-lower-body mode when feet/knees are cropped so missing lower-body landmarks do not create false plank or back warnings.
...
* [x] **`backend/heuristics/pushup.py`:** Calibrated state-machine tracker with form-gated rep counting, structured faults, setup guidance, and per-rep quality metrics (`min_elbow_angle`, `max_elbow_angle`, `min_back_angle`, duration, and fault codes). Pushup calibration now accepts side/3-quarter upper-body visibility, chooses the most reliable visible side, accepts a visible knee when ankles are not reliable, and infers lower-body presence when feet/knees are cropped.
...
* Pushup tracking is intentionally optimized for side and 3-quarter views. Full head-on pushups remain less reliable because elbow depth and back alignment are harder to infer from 2D webcam landmarks.
...
* **Exercise Tuning:** Validate pushup/squat thresholds against real webcam sessions, especially side/3-quarter pushups with feet cropped and mixed lower-body landmark visibility.
